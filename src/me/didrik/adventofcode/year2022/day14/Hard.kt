...
fun main() {
    ...
    val maxDepth = input.flatMap { it.map { it.split(",").last().toInt() } }.maxOrNull()!!
    val grid = Array(1000) { BooleanArray(200)}
    grid.indices.forEach { grid[it][maxDepth + 2] = true }
    for (path in input) {
