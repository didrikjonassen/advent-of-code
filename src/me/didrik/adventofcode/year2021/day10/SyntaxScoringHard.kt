...
fun main() {
    ...
    val incomplete = input.filter { it.corrupted() == 0L }.map { it.incomplete() }.sorted()
    solve(incomplete[incomplete.size/2])
}
...
private fun String.incomplete(): Long {
    val chars = this.toMutableList()
    loop@
    while (chars.isNotEmpty()) {
        for (i in 0 until chars.size - 1) {
    ...
    return chars.reversed().fold(0L) { acc, c -> acc * 5L + open.indexOf(c) + 1 }
}